...
The raid feed is the primary ops surface. The CTO lead watches KOL handles. With `TWITTERAPI_IO_KEY` or `X_BEARER_TOKEN`, the API pulls those timelines and searches for the ticker/CA. Members open `/app/feed`, see the posts, and click **Shill this** — X opens on that post, a mission is claimed, no hunting. A mention of the coin or contract notifies Telegram/Discord immediately so the whole room piles on that URL. The app does not scrape X HTML; it uses a configured API. Without a key, handles can still be watched and the poller starts once the key is set.
